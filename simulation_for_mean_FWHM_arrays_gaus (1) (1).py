...
def run_simulation(directory, top_freq, bot_freq, std_dev,std_dev_2, ensamble_size, photons_per_emitter,total_time ,number_of_pos ,detector_resolution = 1e2,
    ...
    final_pos = coherence_length_result*1.5 #in nm. 1e8 is the largest possible stage pos but dont increase sim beyond 7e5 in practicality
    ...
    for i in stage_position:
        ...
        start4 = timing.time()
        freq_array = assign_gaussian_frequency_soft_switching_under_envelope(detector_resolution,probability_per_bin, total_time, time_arr, photon_number, expected_oscillations,std_dev,top_freq,std_dev_2)
        end4 = timing.time()
...
primary_directory = '/global/scratch/projects/co_utzatgroup/ccobbbruno/25_01_14_job_arr_testing/'

var = int(sys.argv[1])

ensamble_size = 1
#photons per emitter per second
photons_per_emitter =int(1000*var)
...
for i in range(iterations): 
    ...
    start = timing.time()
    directory = os.path.join(primary_directory, f'{photons_per_emitter} pps 4 to 16 linewdiths {i}')
...
np.save('/global/scratch/projects/co_utzatgroup/ccobbbruno/25_01_14_job_arr_testing/stats_tensor' +f' {photons_per_emitter} pps 4 to 16 linewdiths', mean_std_dev_arr )